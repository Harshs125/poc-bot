    ...
    def get(self,request,empid,*args,**kwargs):
        try:
            ...
            if emp.type=="ADMIN":
                    ...
                    # print(queryset)
                    department_ids = EmpDept.objects.filter(emp_id=empid).values_list('dept_id', flat=True)
            ...
            if request.query_params.get("department_id") :
                queryset=queryset.filter(department__id=request.query_params.get("department_id"))
            # print("---->",queryset)
                        ...
                        if cron_trigger is not None:
                            ...
                            try:
                                job = scheduler.add_job(
                                    ...
                                    args=[configurations_int, schedule_id,schedule.interval,schedule.emp.id],
                                    id=str(schedule_id),
                                )
                            except Exception as e:
                                print(f"An error occurred: {e}")
