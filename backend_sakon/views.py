        ...
        try: 
            ...
            if emp.type == "SUPERADMIN":
                ...
                for employee in employees: 
                    ...
                    employee_info.append({"id": employee.id, "name": employee.name,"email": employee.email,"type": employee.type, "org": employee.org.name}) 
                ...
                for employee in employees: 
                    ...
                    employee_info.append({"id": employee.id, "name": employee.name,"email": employee.email,"type": employee.type,"Department": department_names, "org": emp.org.name}) 
            
            elif emp.type == "USER":
                departments = EmpDept.objects.filter(emp=emp.id) 
                department_names = [department.dept.name for department in departments] 
                
                employee_info.append({"id": emp.id, "name": emp.name,"email": emp.email,"type": emp.type,"Department": department_names, "org": emp.org.name})
        ...
        try:    
            ...
            if role == "SUPERADMIN":
                ...
                if departments.count() != len(department_ids):
                    return JsonResponse({"error": "One or more departments do not exist"}, status=400)
                ...
                try:
                    ...
                    for department in departments:
                        empdeptserializer = EmpDeptSerializer(data={"emp": employee.id, "dept": department.id})
                        try:
    ...
    def put(self, request, id):
        try:
            ...
            if emp.type == "USER" and new_departments:
                ...
                current_departments_set = set([dept.dept.id for dept in current_departments])
                ...
                for dept_id in new_departments:
                    if dept_id not in current_departments_set:
                        ...
                        department.employee_count += 1
                        department.save()
            serializer = EmployeesSerializer(emp, data={"name": request.data.get("name"), "email": request.data.get("email")}, partial=True)
            try:
                serializer.is_valid(raise_exception=True)
                serializer.save()
                return JsonResponse({"message": "successfully updated employee information"}, status=200)
            except Exception as e:
                return JsonResponse({"error": str(e)}, status=400)
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
...
class DepartmentAPI(generics.ListAPIView):
    ...
    def get(self, request, id):
        try:
            departments = Department.objects.filter(org=id)
            response_data = []
            for department in departments:
    ...
    def post(self, request):
        ...
        try:
            ...
            if org_id_present:
                pass
                serializer = DepartmentSerializer(data={"name":request.data.get("name"), "org":request.data.get("org"),"employee_count":0}, partial=True)
...
class OrganizationAPI(generics.ListAPIView):
    ...
    def get(self,request,orgid):
        ...
        service_provider_ids = serializer.data[0]['service_providers']
        print(service_provider_ids)
        service_providers = ServiceProvider.objects.filter(id__in=service_provider_ids)
        service_provider_names = list(service_providers.values_list('name', flat=True))
        print(serializer.data[0].get(''))
        response_data=[]
        response_data.append(
            {
                "id":serializer.data[0].get('id'),
                "name":serializer.data[0].get('name'),
                "department_count":serializer.data[0].get('department_count'),
...
class ConfigurationAPI(generics.ListAPIView):
    ...
    def get(self,request,*args,**kwargs):
        ...
        if dept_name is not None:
            queryset=queryset.filter(deptartment__name=dept_name)
        ...
        if page is not None:
            ...
            return self.get_paginated_response(response_data, count, previous_url, next_url)
    ...
    def put(self, request, id, *args, **kwargs):
        ...
        if serializer.is_valid():
            if(schedule.configurations!=request.data.get("configurations")):
                ...
                Configuration.objects.filter(id__in=conf_ids).update(is_scheduled=False,schedule_id=None)
                configurations = Configuration.objects.filter(
                        id__in=request.data.get("configurations")
                    )
                serializer.save()
...
class DownloadSubtaskAPI(APIView):
    ...
    def delete(self,request,id):
        try:
            ...
            download.delete()
            return JsonResponse({
            "message":"Download Report deleted successfully "
            },status=200)
        except DownloadReport.DoesNotExist:
            return JsonResponse({
                "error":"Download Report not found"
            },status=404)
        except Exception as e:
            return JsonResponse({
                "error":"Internal Server Error",
...
class FileValidatorAPI(APIView):
    ...
    def delete(self,request,id):
        try:
            ...
            file.delete()
            return JsonResponse({
            "message":"file validation report deleted successfully "
            },status=200)
        except FileValidationReport.DoesNotExist:
            return JsonResponse({
                "error":"file validation report not found"
            },status=404)
        except Exception as e:
            return JsonResponse({
                "error":"Internal Server Error",
...
class TemplateValidatorAPI(APIView):
    ...
    def delete(self,request,id):
        try:
            ...
            return JsonResponse({
                "error":"template validation report not found"
            },status=404)
        except Exception as e:
            return JsonResponse({
                "error":"Internal Server Error",
...
class UploadValidatorAPI(APIView):
    ...
    def delete(self,request,id):
        try:
            ...
            upload.delete()
            return JsonResponse({
            "message":"uplaod report deleted successfully "
            },status=200)
        except UploadReport.DoesNotExist:
            return JsonResponse({
        ...
        try:
            ...
            if employee is not None:
                ...
                access_payload = jwt.decode(
                    str(token.access_token), key=settings.SECRET_KEY, algorithms=["HS256"]
                )
...
class ChartGraphAPI(APIView):
    def get(self, request):
        ...
        successuploadcount = uploadqueryset.filter(status="Completed", Triggered_At__gte=start_datetime).count()
        failureuploadcount = totalUploads - successuploadcount
        totalDownloads = downloadqueryset.filter(Triggered_At__gte=start_datetime).count()
        successdownloadcount = downloadqueryset.filter(status="Completed", Triggered_At__gte=start_datetime).count()
        ...
        error_jobs = jobsqueryset.filter(status__in=["Pending", "Failed"], Triggered_At__gte=start_datetime)
        error_data = []
        ...
        while current_interval < current_datetime:
            ...
            errors_within_interval = error_jobs.filter(Triggered_At__gte=current_interval, Triggered_At__lt=current_interval + interval_timedelta)
            error_counts["name"] = key
            error_counts["error"] = len(errors_within_interval)
            error_data.append(error_counts)
            current_interval += interval_timedelta  # Add this line to increment current_interval
        return JsonResponse(
            {
                        ...
                        if cron_trigger is not None:
                            ...
                            try:
                                job = scheduler.add_job(
                                    ...
                                    args=[configurations_int, schedule_id,schedule.interval],
                                    id=str(schedule_id),
                                )
                            except Exception as e:
                                print(f"An error occurred: {e}")
